...
struct LuaConstant {
    ...
    LuaConstant(const LuaConstant& other) : type(other.type) {
        switch (type) {
            case LUA_TBOOLEAN: bval = other.bval; break;
            case LUA_TNUMFLT: nval = other.nval; break;
            case LUA_TNUMINT: ival = other.ival; break;
            case LUA_TSHRSTR:
            case LUA_TLNGSTR: new (&sval) std::string(other.sval); break;
            default: break; 
        }
    }
    ...
    LuaConstant(LuaConstant&& other) noexcept : type(other.type) {
        switch (type) {
            ...
            case LUA_TNUMINT: ival = other.ival; break;
            case LUA_TSHRSTR:
            case LUA_TLNGSTR: new (&sval) std::string(std::move(other.sval)); break;
            default: break; 
        }
        other.type = LUA_TNIL; 
    }
...
struct LuaProto {
    ...
    LuaProto() : linedefined(0), lastlinedefined(0), numparams(0), is_vararg(0), maxstacksize(0) {}
};
...
private:
    ...
    lua_Number read_number();
    lua_Integer read_integer();
